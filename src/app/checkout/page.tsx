...
export default function CheckoutPage() {
  ...
  const summary = useMemo(() => {
    const subtotalCalc = groups.reduce(
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      try {
        const resp = await api.checkPincodeServiceability(
  ...
  useEffect(() => {
    ...
    const run = async () => {
      try {
        const resp = await api.getExpectedTat({
    ...
    const options = {
      ...
      handler: async function (response: any) {
        try {
          ...
          // Verify payment on backend and create invoice
          try {
                ...
                <div className="checkout_header" style={{ marginTop: "20px" }}>
                  ...
                  {serviceabilityLoading ? (
                    ...
                    <p style={{ color: "#a00", margin: 0 }}>
                      Unable to verify serviceability right now
                    </p>
                  ) : (
                ...
                >
                  ...
                  <ul>
                    {groups.map((group) => {
                      ...
                      const name =
                        group.product?.name || group.name || "Product";
                      return (
                        <li key={group.key}>
                          <a
                            href="#"
