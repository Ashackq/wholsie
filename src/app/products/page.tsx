...
function ProductsContent() {
  ...
  // Removed summing logic because category counts now include overlapping combos (double counting)
  // Instead we rely on the products API to give us the total count
  /*
    useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (!initialLoadDone) {
      ...
      // Also fetch true total count of active products for "All Products" label
      fetch(
        `${process.env.NEXT_PUBLIC_API_URL || "http://localhost:4000/api"}/products?limit=1`,
      )
        .then((res) => res.json())
        .then((data) => {
          if (data.pagination && typeof data.pagination.total === "number") {
            setAllProductsCount(data.pagination.total);
          }
        })
        .catch(console.error);
  ...
  const {
    ...
    const puffCats = categories.filter(
      (cat) =>
        cat.name?.toLowerCase().includes("puff") ||
        cat.slug?.toLowerCase().includes("puff"),
    );
    const makhanaCat =
      categories.find(
        (cat) =>
          cat.name?.toLowerCase().includes("makhana") ||
          cat.slug?.toLowerCase().includes("makhana"),
      ) || null;
    ...
    const puffCount = puffCats.reduce(
          ...
          {puffCategories.length > 0 && (
            <li
              className={
                (
                  !isMobile
                    ? puffCategories.some((cat) => cat.slug === activeCategory)
                    : puffCategories.some((cat) => cat.slug === tempCategory)
                )
                  ? "current"
                  : ""
              }
            ...
            >
              {isMobile ? (
                <a
                  ...
                  onClick={(e) => {
                    e.preventDefault();
                    if (mainPuffSlug) setTempCategory(mainPuffSlug);
                  }}
                  style={{
                    display: "flex",
                    justifyContent: "space-between",
                ...
                <Link
                  href={
                    mainPuffSlug
                      ? `/products?category=${mainPuffSlug}`
                      : "/products"
                  }
                >
                  <span>Puffs</span>
          ...
          {makhanaCategory && (
            <li
              className={
                (
                  !isMobile
                    ? makhanaCategory.slug === activeCategory
                    : makhanaCategory.slug === tempCategory
                )
                  ? "current"
                  : ""
              }
            ...
            >
              {isMobile ? (
                <a
                  ...
                  onClick={(e) => {
                    e.preventDefault();
                    setTempCategory(makhanaCategory.slug);
                  }}
                  style={{
                    display: "flex",
                    justifyContent: "space-between",
                ...
                >
                  ...
                  >
                    {tempCategory === makhanaCategory.slug && (
                      <i
                        className="fa-solid fa-check"
                        style={{ color: "var(--primary)", fontSize: "12px" }}
                      ></i>
              ...
              <div>
                ...
                {tempCategory && (
                  ...
                  >
                    ...
                    >
                      {categories.find((c) => c.slug === tempCategory)?.name ||
                        tempCategory}
                    </span>
                  </p>
                )}
                    ...
                    {products.length > 0 ? (
                      [...products]
                        ...
                        .sort((a, b) => {
                          ...
                          if (sort === "newest")
                            return (b as any)._id.localeCompare((a as any)._id);
                          return 0;
                        })
                        .map((product) => {
                    ...
                    >
                      <button
                        ...
                        style={{
                          ...
                          cursor:
                            loading || isFetching ? "not-allowed" : "pointer",
                        }}
                      >
