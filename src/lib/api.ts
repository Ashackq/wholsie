...
export async function getProducts(limit = 12, offset = 0, categoryId?: string): Promise<ApiResponse<Product[]>> {
    ...
    return apiCall<ApiResponse<Product[]>>(`/products?${params.toString()}`, { skipAuth: true });
}

export async function getProduct(idOrSlug: string): Promise<ApiResponse<Product>> {
    return apiCall<ApiResponse<Product>>(`/products/${idOrSlug}`, { skipAuth: true });
}
...
export async function addToCart(productId: string, quantity: number, variantId?: string): Promise<ApiResponse<Cart>> {
...
export async function removeFromCart(itemId: string): Promise<ApiResponse<Cart>> {
    return apiCall<ApiResponse<Cart>>(`/cart/items/${itemId}`, { method: "DELETE" });
}
...
export async function deleteOrder(orderId: string): Promise<ApiResponse<{ success: boolean }>> {
...
export async function checkPincodeServiceability(pincode: string): Promise<ApiResponse<PincodeServiceability>> {
    return apiCall<ApiResponse<PincodeServiceability>>("/delhivery/check-pincode", {
        method: "POST",
...
export async function trackShipment(waybill: string): Promise<ApiResponse<TrackingStatus>> {
...
export async function createShipment(data: CreateShipmentRequest): Promise<ApiResponse<ShipmentResponse>> {
...
export async function cancelShipment(waybill: string): Promise<ApiResponse<{ success: boolean; message?: string }>> {
    return apiCall<ApiResponse<{ success: boolean; message?: string }>>("/delhivery/cancel-shipment", {
        method: "POST",
        body: JSON.stringify({ waybill }),
    });
}

export async function getPickupLocations(): Promise<ApiResponse<Array<{
    name: string;
    pin: string;
    add: string;
    city: string;
    ...
    return apiCall<ApiResponse<Array<{
        name: string;
        pin: string;
        add: string;
        city: string;
