 ...
 */
export async function createShipment(shipmentData: DelhiveryShipmentData): Promise<{
    success: boolean;
    waybill?: string;
    packages?: string;
    error?: string;
...
/**
 * Track a shipment using waybill/AWB number
 */
export async function getTrackingStatus(waybill: string): Promise<TrackingData | null> {
    const token = env.DELHIVERY_TOKEN;
    if (!token) {
        throw new Error("DELHIVERY_TOKEN not configured");
    }
    ...
    try {
        const url = `${DELHIVERY_API_BASE}/api/v1/packages/json/?waybill=${encodeURIComponent(waybill)}`;
        const response = await fetch(url, {
            method: "GET",
            headers: {
                Authorization: `Token ${token}`,
 ...
 */
export async function getPickupLocations(): Promise<Array<{
    name: string;
    pin: string;
    add: string;
    city: string;
...
/**
 * Fetch waybill numbers in advance (Bulk Waybill API)
 * Required for pre-assigning waybills to orders
 */
export async function fetchWaybills(count: number = 10): Promise<{
    ...
    try {
        const url = `${DELHIVERY_API_BASE}/waybill/api/bulk/json/?count=${count}`;
        const response = await fetch(url, {
            method: "GET",
            headers: {
                Authorization: `Token ${token}`,
 ...
 */
export async function getEstimatedTAT(originPin: string, destinationPin: string): Promise<{
    success: boolean;
    tat?: number; // in days
    error?: string;
}> {
 ...
 */
export async function updateWarehouse(name: string, updates: Partial<WarehouseData>): Promise<{
    success: boolean;
    message?: string;
    error?: string;
}> {
 ...
 */
export async function updateShipment(waybill: string, updates: {
    name?: string;
    phone?: string;
    address?: string;
    city?: string;
...
export async function createPickupRequest(params: {
    ...
    try {
        ...
        if (params.expectedPackageCount) {
            formData.append("expected_package_count", params.expectedPackageCount.toString());
        }
 ...
 */
export async function downloadDocument(waybill: string, documentType: "pod" | "qc"): Promise<{
    success: boolean;
    documentUrl?: string;
    error?: string;
}> {
