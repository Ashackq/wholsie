...
paymentRouter.post("/payments/order", requireAuth, async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
        ...
        const amountInPaise = Math.round(order.netAmount * 100);
        const RAZORPAY_MIN_AMOUNT = 100; // ₹1 minimum

        if (amountInPaise < RAZORPAY_MIN_AMOUNT) {
            return res.status(400).json({
                error: `Order amount (₹${order.netAmount}) is below minimum (₹1)`
            });
        }
    ...
    try {
        ...
        if (!signature) {
            ...
            if (webhookSecret) {
                ...
                console.warn("RAZORPAY_WEBHOOK_SECRET not configured - webhook signature validation skipped");
            }
        }
        ...
        if (event === "payment.captured") {
            ...
            const { sendPaymentConfirmationEmail, prepareInvoiceData } = await import("../utils/orderEmail.js");
            ...
            if (order) {
                ...
                try {
                    ...
                    console.error(`⚠️  Failed to send payment confirmation email for order ${order.orderId || order._id}:`, emailErr);
                    // Don't fail the payment webhook if email fails
                }

                // Check if auto-shipment is enabled
                const autoShipmentEnabled = await getSetting("autoShipmentEnabled", false);

                if (autoShipmentEnabled && !order.delhiveryTrackingId && order.shippingAddress) {
                    try {
                        console.log(`Auto-creating shipment for order ${order.orderId || order._id}`);
                        ...
                        const productDesc = order.items.map((item: any) => `${item.name} (${item.quantity})`).join(", ");
                        ...
                        if (result.success && result.waybill) {
                            ...
                            console.log(`Auto-shipment created successfully! Waybill: ${result.waybill}`);
                        } else {
                            console.error("Auto-shipment failed:", result.error || result.rmk);
                        }
                    } catch (shipmentErr) {
                ...
                console.warn(`Order not found for Razorpay order ID: ${paymentEntity.order_id}`);
            }
